...
<head>
    ...
    <title>Raffinert Actualizar genero</title>
...
<body>
    <div class="container">
        <?php 
            if(isset($_GET['idgenero'])){
                $objUpdate= new Crud();
                $dataUpdate= $objUpdate->setGeneroId($_GET['idgenero']);
            }
        ?>
        <form action="generoform.php" method="post">
            <input type="hidden" name="veri" id="veri" value="<?= $dataUpdate['idgenero'];?>">
            <div class="row">
                <div class="col-md-12 mt-3 mb-3">
                    <label for="nombre">Actualizar genero</label>
                    <input type="text" class="form-control shadow-sm bg-white rounded" name="nombre" value="<?= $dataUpdate['nombre_genero'];?>" placeholder="Digite nombre del genero" required id="nombre">
                </div>
            </div>
    ...
    <?php 
        if(isset($_POST['submit'])){
            ...
            $dataEnviarUpdate= $objEviarUpdate->getGeneroUpdate($nombre,$veri);
            if($dataEnviarUpdate !== ""){
                echo "<script> 
                        setTimeout(function() { 
                            swal({ 
